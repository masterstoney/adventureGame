...
int main() {
	...
	string firstInput = "";
	string secondInput = "";
	GameCommunication coms;
	GameInstructions ins;
	...
	cout<<user.communicate(matchInput<ConversationInput>(secondInput,9,ConversationInputNames))<<endl;
	phillip.communicate(cout,matchInput<ConversationInput>(secondInput,9,ConversationInputNames),user.gameProgress(),user.getType());

	fgets(clearBuffer,2,stdin);
	...
	while (gameStage != 15) {
		...
		cout<<ins.retrieveInstruction(gameStage)<<endl;
		fgets(userInput,40,stdin);
		remove_new_line(userInput);
		userInputString += charToString(userInput);
		cout<<coms.getConversationPiece(userInputString)<<endl;
		gameStage++;
